...
function RemoteEmailVarify() {
  const [emailState, setemailState] = useState(false);

  const fetchemailState = async () => {
    try {
      const timestamp = new Date().getTime();
      const res = await fetch(`${HTTP_API_END_POINT.mockCheckVerifyEmail}?t=${timestamp}`);
  ...
  return (
    <div className = {styles.container}>
      ...
      <p className={styles.state}>state : {emailState ? '인증됨' : '인증되지 않음'}</p>
      <button className={styles.toggle} onClick={handleToggle}>인증 상태 변경</button>
    </div>
  );
}

export default RemoteEmailVarify;
